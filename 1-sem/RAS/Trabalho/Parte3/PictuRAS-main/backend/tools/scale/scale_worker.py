...
REQUEST_QUEUE = os.getenv('REQUEST_QUEUE', 'scale-queue')
...
class ScaleWorker:
    ...
    def on_request(self, ch, method, properties, body):
        self.pool.apply_async(ScaleWorker.worker_handle_request, (ch, method, properties, body))


    def worker_handle_request(ch, method, properties, body):

        print(f'ScaleWorker received image: {properties.correlation_id}')
        request = ScaleMessageRequest.from_json(body.decode())
        tool = ScaleTool(request)
        response = tool.apply().to_json()

        ch.connection.add_callback_threadsafe(
            functools.partial(ScaleWorker.publish_response, ch, properties, response))

        ch.connection.add_callback_threadsafe(
            functools.partial(ScaleWorker.ack_message, ch, method.delivery_tag))
