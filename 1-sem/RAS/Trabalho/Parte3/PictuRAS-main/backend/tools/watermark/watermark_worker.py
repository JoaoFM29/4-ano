...
class WatermarkWorker:
    ...
    def on_request(self, ch, method, properties, body):
        self.pool.apply_async(WatermarkWorker.worker_handle_request, (ch, method, properties, body))
    ...
    def worker_handle_request(ch, method, properties, body):
        ...
        ch.connection.add_callback_threadsafe(
            functools.partial(WatermarkWorker.publish_response, ch, properties, response))

        ch.connection.add_callback_threadsafe(
            functools.partial(WatermarkWorker.ack_message, ch, method.delivery_tag))
    ...
    def publish_response(ch, properties, response):
        ...
        print(f'WatermarkWorker sent image: {properties.correlation_id}')
