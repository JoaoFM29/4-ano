...
class Node:
    def __init__(self, node_ip, rtsp_port, rtp_port, node_id, node_type, control_port=50051, data_port=50052, bootstrapper_host='localhost', bootstrapper_port=5000):
        ...
        print(f"Escutando em {self.node_ip}:{self.rtsp_port} RTSP")
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if header == b'\x01':  # ControlMessage
                ...
                if response_message.type == ControlMessage.REGISTER_RESPONSE:
                    print(f"{self.node_id} registered")
                    ...
                    print(f"{self.node_id} neighbors: {self.neighbors}")
            ...
            try:
                ...
                if neighbor_info["node_id"].startswith("client"):
                    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                        ...
                        print(f"Notified {neighbor_info['node_id']} of registration.")
                ...
                print(f"Failed to notify {neighbor_info['node_id']}: {e}")
    ...
    def control_server(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            s.listen()
            print(f"{self.node_id} listening on control port {self.control_port}")
            while True:
                conn, addr = s.accept()
    ...
    def handle_update_neighbors(self, control_message):
        ...
        if neighbor_ip in neighbors_snapshot:
            ...
            print(f"Updated status of {neighbor_id} to active.")
        else:
            with self.neighbors_lock:
        ...
        while True:
            ...
            for neighbor_ip, neighbor_info in neighbors_snapshot.items():
                ...
                if neighbor_info.get("failed-attempts", 0) >= 2:
                    print(f"{neighbor_info['node_id']} inativo por falha de PONG")
                    with self.neighbors_lock:
                    ...
                    with self.sessions_lock:
                        for filename, session_data in list(self.sessions.items()):
                            if neighbor_ip in session_data:
                                self.sessions[filename][neighbor_ip]["rtpSocket"].close()
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                if best_received_neighbor is not None:
                    print(f"Tempo acumulado {best_received_accumulated_time:.4f} - {best_received_neighbor}")
                ...
                print(f"Sent PING to {neighbor_info['node_id']} with {new_accumulated_time:.4f}")
    ...
    def activate_best_route(self, flooding_message): # Sender diz se é uma ativação do cliente ou se é do node
        """
        ...
        if best_route is not None:
            ...
            with self.sessions_lock:
                ...
                if not flooding_message.auxAtivation and 'flow' not in self.sessions[filename][flooding_message.source_ip]:
            ...
            if forward_activation:
                print(f"Activating best route to {best_route['source_id']} with {min_time:.4f} time.")
                ...
                if flooding_message.auxAtivation is not None:
                    activate_message.auxAtivation = flooding_message.auxAtivation
                    
                try:
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                        s.connect((destination, best_route['control_port']))
                        self.send_flooding_message_tcp(s, activate_message)
                        print(f"Sent route activation to {best_route['source_id']} - {filename}.")
                            
                except Exception as e:
                    print(f"Failed to activate route to {best_route['source_id']}: {e}")
            ...
            if route_ip != dest_ip and filename in route_info:
                ...
                if route_info[filename]["stream"] == "active" or route_info[filename]["flow"] == "active":
                    ...
                    with self.neighbors_rtsp_lock:
                        if route_ip in self.neighbors_rtsp and 'rtsp_socket' in self.neighbors_rtsp[route_ip]:
    ...
    def replace_ip_in_request(self, request, new_ip):
    ...
    def handle_neighbor(self, neighbor_socket, neighbor_address):
        ...
        while True:
            try:
                ...
                if not request:
                    break
                print(f"Requisição recebida de {neighbor_address}:\n{request}\n")

                # Retira as informações necessárias da requisição
                lines = request.splitlines()
                line1 = lines[0].split(' ')
                filename = line1[1]
                request_ip = self.get_ip_from_request(request)                 
                ...
                if "SETUP" in request:  
                    if self.route_with_SETUP(filename) or self.at_least_one_receiving_rtp(filename, None): # Caso já tenha recebido algum setup ou já esteja a enviar dados a alguem
                        ...
                        modified_request = self.replace_ip_in_request(request, self.node_ip)     
    ...
    def route_with_SETUP(self, stream_id):
        ...
        for route_ip, route_info in routing_snapshot.items():
            ...
            if stream_id in route_info and 'request' in route_info[stream_id]:
                if route_info[stream_id]['request'] == "SETUP" or route_info[stream_id]['request'] == "PLAY":
                    return True
        return False
    ...
    def remove_connection(self, filename, neighbor_address, request, neighbor_socket):         
        ...
        with self.sessions_lock: 
            ...
            del self.sessions[filename][neighbor_address] 
            sessions = len(self.sessions[filename])
            
        if sessions > 0:
            if self.at_least_one_receiving_rtp(filename, None):
                lines = request.splitlines()
                line1 = lines[0].split(' ')
                seq = lines[1].split()[1]
                self.replyRtsp(seq, filename, neighbor_socket)
            
            else:
                modified_request = self.replace_ip_in_request(request, self.node_ip)  
                dest = self.forward_request(filename, modified_request, neighbor_socket) 
                with self.routing_lock:
                    self.routing_table[dest][filename]['stream'] = "deactive"
                    del self.routing_table[dest][filename]['flow']
                    del self.routing_table[dest][filename]['request']
                    
                with self.neighbors_rtsp_lock:
                    if dest in self.neighbors_rtsp and 'rtsp_socket' in self.neighbors_rtsp[dest]:
                        self.neighbors_rtsp[dest]['rtsp_socket'].close()
                        del self.neighbors_rtsp[dest]['rtsp_socket']
        else:   
        ...
        print("DADOS DO VIZINHO APAGADOS COM SUCESSO")
    ...
    def data_server(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            print(f"{self.node_id} listening on data port {self.data_port}")
            while True:
    ...
    def handle_ack(self, s, control_message):
        ...
        try:
            ...
            print(f"ACK sent to {control_message.node_id} with {best_time:.4f} time")
