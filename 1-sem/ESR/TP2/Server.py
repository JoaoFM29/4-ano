...
class Server:	
    def __init__(self,server_ip, server_id, control_port=50051, data_port=50052, server_rtsp_port=30000, bootstrapper_host='localhost', bootstrapper_port=5000):
        ...
        print(f"Escutando em :{self.server_rtsp_port} RTSP")
    ...
    def handle_update_neighbors(self, control_message):
        ...
        print(f"{self.server_id} neighbors: {self.neighbors}")
        ...
        while True:
            ...
            for neighbor_ip, neighbor_info in neighbors_snapshot.items():
                ...
                if neighbor_info.get("failed-attempts", 0) >= 2:
                    print(f"Neighbor {neighbor_info['node_id']} inativo por falha de PONG.")
                ...
                try:
                    ...
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                        ...
                        print(f"Sent PING to {neighbor_info['node_id']}")
                    ...
                    print(f"Failed to send PING to {neighbor_info['node_id']}: {e}")
    ...
    def handle_rtsp_connection(self, rtsp_socket, requestType, filename, sender_ip, seq):
        ...
        try:
            ...
            if existing_worker:
                ...
                print(f"Sessão atualizada para o fluxo {filename} com o vizinho {sender_ip}:{new_rtp_port}")
                ...
                print(f"Criando sessão para o fluxo {filename} com o vizinho {sender_ip}:{new_rtp_port}")
    ...
    def handle_rtsp_connection2(self, rtsp_socket, filename, sender_ip, rtp_port):
        ...
        try:
            ...
            if existing_worker:
                ...
                print(f"Sessão atualizada para o fluxo {filename} com o vizinho {sender_ip}:{rtp_port}")
            else:
                # Cria um novo ServerWorker para o fluxo
                neighborInfo = {
                    "ip": sender_ip,
                ...
                print(f"Criando sessão para o fluxo {filename} com o vizinho {sender_ip}:{rtp_port}")
                worker = ServerWorker(neighborInfo, self.server_ip)
                with self.active_workers_lock:
                    self.active_workers[filename] = worker
        except Exception as e:
