...
class Client:
    ...
    def start_new_session(self):
        ...
        while True:
            ...
            best_route = self.activate_best_route(self.filename, video_session)
            if best_route:
                ...
                with self.dest_lock:
                    ...
                    if dest_ip != new_ip or dest_rtspport != new_rtsp_port:
    ...
    def activate_best_route(self, filename, video_session):
        """
        ...
        if best_route is not None:
            ...
            print(f"Best route is {best_route['node_id']}: {min_time:.4f}s.")
            ...
            if video_session is not None:
                activate_message.auxAtivation = video_session.active
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                    ...
                    print(f"Sent route activation to {best_route['node_id']}")
            ...
            if route_info["stream"] == "active" and route_ip != destination: 
                ...
                try:
                    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                        ...
                        print(f"Sent route deactivation to {route_info['node_id']}")
                except Exception as e:
    ...
    def handle_update_neighbors(self, control_message):
        ...
        if neighbor_ip in neighbors_snapshot:
            with self.neighbors_lock:
                ...
                self.neighbors[neighbor_ip]["best_time"] = float('inf')
            ...
            for neighbor_ip, neighbor_info in neighbors_snapshot.items():
                ...
                try:
                    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                        ...
                        #Aumenta sempre que envia porque udp nao garante a entrega
                        with self.neighbors_lock: 
