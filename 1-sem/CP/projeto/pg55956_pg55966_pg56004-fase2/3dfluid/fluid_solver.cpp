...
void add_source(int M, int N, int O, float *x, float *s, float dt) {
  ...
  #pragma omp parallel for shared(x,s)
...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
    ...
    do {
        ...
        // Red phase
        #pragma omp parallel for shared(x, x0) reduction(max:max_c) private(old_x,change)
        for (int k = 1; k <= O; k++) {
            for (int j = 1; j <= N; j++) {
...
void advect(int M, int N, int O, int b, float *d, float *d0, float *u, float *v,
  ...
  #pragma omp parallel for shared(u,v,w,d,d0,dtX,dtY,dtZ)
  for (int k = 1; k <= O; k++) {
    for (int j = 1; j <= N; j++) {
      for (int i = 1; i <= M; i++) {
...
void project(int M, int N, int O, float *u, float *v, float *w, float *p,
  ...
  #pragma omp parallel for shared(div,u,v,w,p)
  for (int k = 1; k <= O; k++) {
    for (int j = 1; j <= N; j++) {
      for (int i = 1; i <= M; i++) {
        div[IX(i, j, k)] =
  ...
  #pragma omp parallel for shared(u,v,w,p)
  for (int k = 1; k <= O; k++) {
    for (int j = 1; j <= N; j++) {
      for (int i = 1; i <= M; i++) {
